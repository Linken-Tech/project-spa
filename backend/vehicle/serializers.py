...
class VehicleSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if car_doc:
            create_vehicle_doc = create_vehicle.documents.bulk_create(
                [
        ...
        if car_images:
            create_vehicle_image = create_vehicle.vehicle_image.bulk_create(
    ...
    def update(self, instance, validated_data):
        ...
        if car_doc:
            update_vehicle_doc = instance.documents.bulk_create(
                [VehicleDocument(document=doc, vehicle=instance) for doc in car_doc]
            )
        if car_images:
            update_vehicle_image = instance.vehicle_image.bulk_create(
